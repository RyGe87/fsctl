// Carried over from sshctl, deliberately whole: the parts fsctl does not use
// yet (tabs, wrapping) stay so the two copies keep diverging as little as
// possible and a fix can travel by hand between them.
#![allow(dead_code)]
...
impl Color {
    /// The classic codes, deliberately. crossterm sent `38;5;N` palette
...
impl Block {
    ...
    pub fn render_frame(&self, area: Rect, frame: &mut Frame) -> Rect {
...
struct Cell {
    ch: char,
    /// A combining mark that belongs to `ch`. macOS stores file names
    /// decomposed, so "café" arrives as five characters and the accent has to
    /// ride along in the same cell or the whole row shifts.
    comb: Option<char>,
    style: Style,
}
...
impl Frame {
    ...
    /// Hangs a combining mark on the character already in this cell.
    pub fn add_combining(&mut self, x: u16, y: u16, mark: char) {
        if x >= self.width || y >= self.height {
            return;
        }
        let cell = &mut self.cells[usize::from(y) * usize::from(self.width) + usize::from(x)];
        // Only the first mark is kept: a cell is one glyph, and file names
        // stacking two accents on one letter are not worth a Vec per cell.
        if cell.comb.is_none() {
            cell.comb = Some(mark);
        }
    }
}

/// Written into the cell that a double-width character spills into. The
/// renderer emits nothing for it — the terminal has already moved two columns
...
impl DefaultTerminal {
    pub fn draw(&mut self, render: impl FnOnce(&mut Frame)) -> std::io::Result<()> {
        ...
        for y in 0..usize::from(self.height) {
            ...
            for cell in row {
                // The spill cell of a wide character: the terminal is already
...
mod tests {
    ...
    /// this pins the winner so nobody "fixes" it back.
    #[test]
    fn colours_travel_as_the_classic_codes_the_eye_chose() {
        let style = Style::new().fg(Color::Cyan).add_modifier(Modifier::BOLD);
        assert_eq!(style.sgr(), "\x1b[0;1;36m");
        let dim = Style::new().fg(Color::DarkGray);
        assert_eq!(dim.sgr(), "\x1b[0;90m");
        let tab = Style::new().add_modifier(Modifier::BOLD).bg(Color::Blue);
        assert_eq!(tab.sgr(), "\x1b[0;1;44m");
    }
